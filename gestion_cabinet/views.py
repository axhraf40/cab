...
def dashboard(request):
    ...
    if hasattr(user, 'dentiste_profile'):
        # Debug information
        print(f"User Role: {user.role}")
        print(f"User ID: {user.id}")
        print(f"Dentiste Profile: {user.dentiste_profile}")
        
        context['rendez_vous_prevus'] = RendezVous.objects.filter(
        ...
        context['factures'] = Facture.objects.filter(
            consultation__rendez_vous__patient=user.patient_profile
        )
...
def consultation_create(request, rendez_vous_id):
    rendez_vous = get_object_or_404(RendezVous, id=rendez_vous_id, dentiste=request.user.dentiste)
...
def secretaire_create(request):
    if not hasattr(request.user, 'dentiste_profile'):
        messages.error(request, "Seuls les dentistes peuvent créer des secrétaires.")
        return redirect('dashboard')
    ...
    if request.method == 'POST':
        ...
        form = UserRegistrationForm(initial={'role': 'SECRETAIRE'})
